...
function init() {
  ...
  function loadMonitorModel() {
    ...
    monitorLoader.load("/models/monitor.glb", (gltf) => {
      ...
      screenMesh = monitor.getObjectByName("Cylinder003_Material004_0") as THREE.Mesh | undefined;
  ...
  function updateEyeRotation() {
    ...
    if (monitorGroup) {
      ...
      monitorQuat.setFromEuler(
        new THREE.Euler(monitorCurrentRotationX, monitorCurrentRotationY, 0, "YXZ"),
      );
